...
from ISAB import MultiheadAttentionBlock  # ISABは使わず、MABのみ

class MAB_VQVAE(nn.Module):
    def __init__(self, num_points=5000, dim_input=3, dim_hidden=128, num_heads=4,
                 embedding_dim=3, num_embeddings=1024):
        super(MAB_VQVAE, self).__init__()
        self.num_points = num_points
        self.dim_input = dim_input
        self.embedding_dim = embedding_dim
        self.num_embeddings = num_embeddings

        # MAB Encoder（Q=K=入力とする自己注意型）
        self.mab = MultiheadAttentionBlock(dim_input, dim_input, dim_hidden, num_heads)
